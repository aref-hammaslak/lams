...
import { CalibrationAPI } from "../../../apis/CalibrationAPI.js";
...
const CalibrationDefaultValues = (calibration) => {
...
function Calibration() {
	...
	const onSubmit = ({ _id, ...data }) => {
		...
		if (_id) {
			CalibrationAPI.update(_id, data).then(
				(calibration) => {
					setCalibrations(
						calibrations.map((t) =>
							t._id === calibration._id ? calibration : t
						)
					);
					const defaultValues = CalibrationDefaultValues(calibration);
					reset(defaultValues);
					setOpen(false);
				},
				(err) => setError(err)
			...
			CalibrationAPI.create(data).then(
				(calibration) => {
					setCalibrations([calibration, ...calibrations]);
					setOpen(false);
				},
				(err) => setError(err)
			);
	...
	const handleDelete = () => {
		...
		CalibrationAPI.destroy(_id).then(
			(calibration) => {
				setCalibrations(
					calibrations.filter((t) => t._id !== calibration._id)
				);
				setOpen(false);
			},
			(err) => setError(err)
				...
				renderCell: ({ row }) => (
					<>
						<Button
							onClick={() => {
								console.log("row", row);
								reset(CalibrationDefaultValues(row));
								handleOpen();
							}}
						>
							<EditIcon />
	...
	useEffect(() => {
		CalibrationAPI.getAll().then(
			(calibrations) => setCalibrations(calibrations),
			(err) => setError(err)
		);
		EquAPI.getAll().then(
		...
		<>
			...
			>
				...
				>
					...
					<Typography color="text.primary">Calibration</Typography>
				</Breadcrumbs>
				<MUILink
			...
			>
				<DialogTitle>
					{watch("name") ||
						(watch("_id") ? "Edit Calibration" : "New Calibration")}
				</DialogTitle>
				<DialogContent>
					...
					<Grid container width="inherit" mt="1rem" rowSpacing="2rem">
						<Grid item xs={12}>
							<RHFAutocomplete
								...
								rules={{ required: true }}
								autocompleteProps={{
									// isOptionEqualToValue={(op, val)=> op === val.eq_id},
